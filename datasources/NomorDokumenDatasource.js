class NomorDokumenDatasource{
    ...
    async addDokumen(input){
        const sql = `INSERT INTO nomor_dokumen(tipe_dokumen,kode_project) VALUES ('${input.tipe_dokumen}','${input.projects}')`;
        const sqlLastNomor = `SELECT*FROM nomor_dokumen WHERE sequence = (SELECT MAX(sequence) FROM nomor_dokumen)`;
        try{
            await this.moshelpPGDB.sequelize.query(sql,null,{raw:true}); //insert
